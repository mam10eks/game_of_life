...
utilityFunctionsSuite =
    Test.describe "Test-Suite to verify the behaviour of utility functions" [
        Test.test "Check that mapCellComponentWithBreak returns input" <| \_ -> Expect.equal 2 (Main.mapCellComponentWithBreak 2 4)

        , Test.test "Check that mapCellComponentWithBreak returns border" <| \_ -> Expect.equal 3 (Main.mapCellComponentWithBreak -1 4)

        , Test.test "Check that mapCellComponentWithBreak returns start" <| \_ -> Expect.equal 0 (Main.mapCellComponentWithBreak 4 4)

        , Test.test "Check that mapCellWithBreak returns input (1,1)" <| \_ -> Expect.equal (1,1) (Main.mapCellWithBreak (1,1) testModel)

        , Test.test "Check that mapCellWithBreak returns input (0,0)" <| \_ -> Expect.equal (0,0) (Main.mapCellWithBreak (0,0) testModel)

        , Test.test "Check that mapCellWithBreak breaks for (-1,-1)" <| \_ -> Expect.equal (3,3) (Main.mapCellWithBreak (-1,-1) testModel)

        , Test.test "Check that mapCellWithBreak breaks for (1,-1)" <| \_ -> Expect.equal (1,3) (Main.mapCellWithBreak (1,-1) testModel)

        , Test.test "Check that mapCellWithBreak breaks for (-1,1)" <| \_ -> Expect.equal (3,1) (Main.mapCellWithBreak (-1,1) testModel)

        , Test.test "Check that mapCellWithBreak breaks for (4,4)" <| \_ -> Expect.equal (0,0) (Main.mapCellWithBreak (4,4) testModel)

        , Test.test "Check that mapCellWithBreak breaks for (1,4)" <| \_ -> Expect.equal (1,0) (Main.mapCellWithBreak (1,4) testModel)

        , Test.test "Check that mapCellWithBreak breaks for (4,1)" <| \_ -> Expect.equal (0,1) (Main.mapCellWithBreak (4,1) testModel)

        , Test.test "Check that for illegal input (-1, -1) an empty set is returned." <|
            \_ -> Expect.equal Set.empty (Main.adjacentCells (-1, -1) testModel)
        ...
        , Test.test "Check that for illegal input (1, 4) an empty set is returned." <|
            \_ -> Expect.equal Set.empty (Main.adjacentCells (1, 4) testModel)

        , Test.test "Check that for illegal input (4, 1) an empty set is returned." <|
            \_ -> Expect.equal Set.empty (Main.adjacentCells (4, 1) testModel)

        , Test.test "Check that adjacent cells without break are correct" <|
            \_ -> Expect.equal (Set.fromList [(0,0), (0,1), (0,2), (1,0), (1,2), (2,0), (2,1), (2,2)]) (Main.adjacentCells (1, 1) testModel)

        , Test.test "Check that adjacent cells with line break for input (0,0) are correct" <|
            \_ -> Expect.equal (Set.fromList [(0,1), (0,3), (1,0), (1,1), (1,3), (3,0), (3,1), (3,3)]) (Main.adjacentCells (0,0) testModel)

        , Test.test "Check that adjacent cells with line break for input (1,0) are correct" <|
            \_ -> Expect.equal (Set.fromList [(0,0), (0,1), (0,3), (1,1), (1,3), (2, 0), (2,1), (2,3)]) (Main.adjacentCells (1,0) testModel)

        , Test.test "Check that dead cell without living neighbours has zero living neighbours" <|
            \_ -> Expect.equal 0 (Main.determineCountOfLivingNeighbours (0, 0) testModel)

        , Test.test "Check that living cell without living neighbours has zero living neighbours" <|
            \_ -> Expect.equal 0 (Main.determineCountOfLivingNeighbours (0, 0) {testModel| livingCells= Set.fromList[(0,0)]})

        , Test.test "Check that dead cell with one living neighbour has one living neighbours" <|
            \_ -> Expect.equal 1 (Main.determineCountOfLivingNeighbours (1, 0) {testModel| livingCells= Set.fromList[(0,0)]})

        , Test.test "Check that living cell with one living neighbour has one living neighbours" <|
            \_ -> Expect.equal 1 (Main.determineCountOfLivingNeighbours (1, 0) {testModel| livingCells= Set.fromList[(0,0), (1,0)]})

        , Test.test "Check that living cell with eight living neighbour has eight living neighbours" <|
            \_ -> Expect.equal 8 (Main.determineCountOfLivingNeighbours (0, 0)
        ...
        , Test.test "Check that in a fully living grid a cell has eight living neighbours" <|
            \_ -> Expect.equal 8 (Main.determineCountOfLivingNeighbours (1, 1)
        ...
        , Test.test "Check that next generation is created fine for a single row as Input" <|
            \_ -> Expect.equal (Set.fromList [(0,0), (0,1), (0,2), (0,3), (1,0), (1,1), (1,2), (1,3), (2,0), (2,1), (2,2), (2,3)])
                (Main.determineNextGeneration {testModel| livingCells = Set.fromList [(1,0), (1,1), (1,2), (1,3)]})
        ]
